...
static bool gs(std::uintptr_t t,std::uint32_t,std::uintptr_t* out){
...
static bool run(const char* family, std::uint32_t mainOr,std::uint32_t mainClear,std::uint32_t mainCount,
  ...
  auto requested=(key & ~0x80400000u)|(expectedKind==4?0x02000000u:0x02000002u);
...
int main(){
  assert(RequestedAlbedoKey(0x02000020u,MaterialAdmission::Foliage)==0x02000020u);
  assert(RequestedAlbedoKey(0x82000020u,MaterialAdmission::Foliage)==0x02000020u);
  assert(CanonicalTechniqueKey(0x02000000u,0x3Cu,RequestedAlbedoKey(0x02000020u,MaterialAdmission::Foliage))==0x02000000u);
  assert(RequestedAlbedoKey(0x02000022u,MaterialAdmission::Foliage)==0x02000022u);
  assert(RequestedAlbedoKey(0x02000020u,MaterialAdmission::Standard)==0x02000022u);
  // Other family selection and range behavior remain covered.
  ...
  DrawRange r{};
  assert(!ShouldWarmRepeatHairRange(r));
  r.familyCounts[1]=127; assert(!ShouldWarmRepeatHairRange(r));
  r.familyCounts[5]=1; assert(ShouldWarmRepeatHairRange(r));
  r.familyCounts[5]=6; assert(ShouldWarmRepeatHairRange(r));
  return 0;
}
