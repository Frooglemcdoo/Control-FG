...
def main():
    ...
    report={'build':'RTX50 HDR Button R21','baseline_zip_sha256':BASE_SHA,'baseline_core_sha256':CORE_SHA,
    ...
    (out/'HDR-BUTTON-R21-BUILD.json').write_text(json.dumps(report,indent=2))
    ...
    insert="""        foreach ($runPid in $runPids.Keys) {
            $hdrButtonLogs=@(Get-ChildItem -LiteralPath $LogDirectory -File -Filter ('hdr-button-R21-*-'+$runPid+'.log') -ErrorAction SilentlyContinue | Where-Object { Test-RegularFile $_ } | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1)
            foreach ($hdrButtonLog in $hdrButtonLogs) { Copy-CompactFile $hdrButtonLog ('hdr-button/'+$hdrButtonLog.Name) 512KB $true }
        }
        $hdrBuild=Get-Item -LiteralPath (Join-Path $ProjectDirectory 'HDR-BUTTON-R21-BUILD.json') -ErrorAction SilentlyContinue
        if (Test-RegularFile $hdrBuild) { Copy-CompactFile $hdrBuild 'package/HDR-BUTTON-R21-BUILD.json' 64KB }
"""
