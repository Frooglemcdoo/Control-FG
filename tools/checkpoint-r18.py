"""Validate and package the r18 full source, without claiming Windows execution."""
from pathlib import Path
import hashlib,json,subprocess,sys,zipfile
root=Path(__file__).resolve().parents[1]
for script in ('validation/g12-current/run.py','validation/part1/run.py','validation/live-guides/run.py','validation/live-capture/run.py','validation/vertex-evidence/run.py','validation/foliage-variant/run.py','validation/eye-variants/run.py'):
 subprocess.run([sys.executable,str(root/script)],check=True)
if len(sys.argv)!=4: raise SystemExit('Usage: checkpoint-r18.py PATH_TO_R5_LOG_ZIP ORIGINAL_D3D_DLL ORIGINAL_RENDERER_DLL')
...
subprocess.run([sys.executable,str(root/'validation/jitter-native/run.py'),sys.argv[3]],check=True)
subprocess.run([sys.executable,str(root/'tools/audit-eye-table.py'),sys.argv[3]],check=True)
digest=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
baseline=root.parent/'g12-r17'
...
allowed={'src/probe.cpp','src/rr_albedo_prepare.h','src/rr_albedo_capture.h','src/rr_diffuse_replay.h'}
unchanged=[]
assert baseline.is_dir(), "r15 baseline is required"
if baseline.exists():
...
assert "Version = '1.0.0-RR-Native-G12-r18'" in metadata
...
summary={'version':'1.0.0-RR-Native-G12-r18','source_revision':'r18-rr-native-g12',
 ...
 'runtime_change':'Eye native-table unique-candidate selection behind strict VS/PS validation; missing-technique identity retained; RR disabled',
 'gpu_reflectance':'r15 observed runtime pass: live capture and numerical sanity passed; coverage unproven. r18 Windows execution pending','evaluation_entry_host_tests':'normal and ASan/UBSan PASS','live_emitter_retirement_slot_tests':'normal and ASan/UBSan PASS','windows_msvc':'NOT_RUN','powershell_parser':'NOT_RUN_ON_THIS_HOST','runtime':'NOT_RUN',
...
out=root.parent/'Control-FG-v1.0.0-RR-Native-G12-r18-Full-Source.zip'
