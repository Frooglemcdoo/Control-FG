"""Validate and package the r17 full source, without claiming Windows execution."""
from pathlib import Path
import hashlib,json,subprocess,sys,zipfile
root=Path(__file__).resolve().parents[1]
for script in ('validation/g12-current/run.py','validation/part1/run.py','validation/live-guides/run.py','validation/live-capture/run.py','validation/vertex-evidence/run.py','validation/foliage-variant/run.py'):
 subprocess.run([sys.executable,str(root/script)],check=True)
if len(sys.argv)!=4: raise SystemExit('Usage: checkpoint-r17.py PATH_TO_R5_LOG_ZIP ORIGINAL_D3D_DLL ORIGINAL_RENDERER_DLL')
...
allowed={'src/probe.cpp','src/rr_albedo_prepare.h'}
unchanged=[]
...
assert "Version = '1.0.0-RR-Native-G12-r17'" in metadata
...
summary={'version':'1.0.0-RR-Native-G12-r17','source_revision':'r17-rr-native-g12',
 ...
 'runtime_change':'Foliage variant request preserves original leaf-wind bit; strict VS identity and all other gates unchanged; RR disabled',
 'gpu_reflectance':'r15 observed runtime pass: live capture and numerical sanity passed; coverage unproven. r17 Windows execution pending','evaluation_entry_host_tests':'normal and ASan/UBSan PASS','live_emitter_retirement_slot_tests':'normal and ASan/UBSan PASS','windows_msvc':'NOT_RUN','powershell_parser':'NOT_RUN_ON_THIS_HOST','runtime':'NOT_RUN',
...
out=root.parent/'Control-FG-v1.0.0-RR-Native-G12-r17-Full-Source.zip'
