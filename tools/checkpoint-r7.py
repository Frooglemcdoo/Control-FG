"""Validate and package the r6 full source, without claiming Windows execution."""
from pathlib import Path
import hashlib,json,subprocess,sys,zipfile
root=Path(__file__).resolve().parents[1]
for script in ('validation/g12-current/run.py','validation/part1/run.py'):
 subprocess.run([sys.executable,str(root/script)],check=True)
if len(sys.argv)!=3: raise SystemExit('Usage: checkpoint-r7.py PATH_TO_R5_LOG_ZIP ORIGINAL_D3D_DLL')
...
assert "Version = '1.0.0-RR-Native-G12-r7'" in metadata
...
summary={'version':'1.0.0-RR-Native-G12-r7','source_revision':'r7-rr-native-g12',
 ...
 'runtime_change':'Native pre-evaluation gateway installed at both exact-build NGX tails; current-call parameters read synchronously; native evaluation forwarded once unchanged',
 'gpu_reflectance':'r6 snapshot implementation unchanged; r6 runtime PASS in supplied logs; new gateway Windows execution pending','evaluation_entry_host_tests':'normal and ASan/UBSan PASS','windows_msvc':'NOT_RUN','powershell_parser':'NOT_RUN_ON_THIS_HOST','runtime':'NOT_RUN',
 'rr_evaluation_enabled':False,'gpu_resource_acquisition':True,'lifetime_proven':False,
 'lifetime_design':'source AddRef retained with job until fence export/retirement; retained indefinitely on recording/submission ambiguity'}
...
out=root.parent/'Control-FG-v1.0.0-RR-Native-G12-r7-Full-Source.zip'
