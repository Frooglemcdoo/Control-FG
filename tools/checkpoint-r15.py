"""Validate and package the r15 full source, without claiming Windows execution."""
from pathlib import Path
import hashlib,json,subprocess,sys,zipfile
root=Path(__file__).resolve().parents[1]
for script in ('validation/g12-current/run.py','validation/part1/run.py','validation/live-guides/run.py','validation/live-capture/run.py'):
 subprocess.run([sys.executable,str(root/script)],check=True)
if len(sys.argv)!=4: raise SystemExit('Usage: checkpoint-r15.py PATH_TO_R5_LOG_ZIP ORIGINAL_D3D_DLL ORIGINAL_RENDERER_DLL')
...
allowed={'src/probe.cpp','src/rr_live_guides.h','src/rr_live_retire.h'}
unchanged=[]
if baseline.exists():
...
assert "Version = '1.0.0-RR-Native-G12-r15'" in metadata
...
summary={'version':'1.0.0-RR-Native-G12-r15','source_revision':'r15-rr-native-g12',
 ...
 'runtime_change':'One-time live owned output readback after native queue fence; copied from same jitter-corrected dispatch; RR disabled',
 'gpu_reflectance':'r12 stability pass: 1915 diffuse copies, 2112 live retirements, no stop. r15 Windows execution pending','evaluation_entry_host_tests':'normal and ASan/UBSan PASS','live_emitter_retirement_slot_tests':'normal and ASan/UBSan PASS','windows_msvc':'NOT_RUN','powershell_parser':'NOT_RUN_ON_THIS_HOST','runtime':'NOT_RUN',
...
out=root.parent/'Control-FG-v1.0.0-RR-Native-G12-r15-Full-Source.zip'
