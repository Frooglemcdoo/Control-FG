"""Validate and package the r13 full source, without claiming Windows execution."""
...
if len(sys.argv)!=3: raise SystemExit('Usage: checkpoint-r13.py PATH_TO_R5_LOG_ZIP ORIGINAL_D3D_DLL')
...
allowed={'src/probe.cpp','src/rr_live_guides.h','src/rr_evaluation_entry.h'}
unchanged=[]
if baseline.exists():
...
assert "Version = '1.0.0-RR-Native-G12-r13'" in metadata
...
summary={'version':'1.0.0-RR-Native-G12-r13','source_revision':'r13-rr-native-g12',
 ...
 'runtime_change':'Native jitter/reset/frame metadata propagated to each live slot; matching frame required before recording; projection rechecked; jitter correction and RR evaluation remain disabled',
 'gpu_reflectance':'r12 stability pass: 1915 diffuse copies, 2112 live retirements, no stop. r13 Windows execution pending','evaluation_entry_host_tests':'normal and ASan/UBSan PASS','live_emitter_retirement_slot_tests':'normal and ASan/UBSan PASS','windows_msvc':'NOT_RUN','powershell_parser':'NOT_RUN_ON_THIS_HOST','runtime':'NOT_RUN',
...
out=root.parent/'Control-FG-v1.0.0-RR-Native-G12-r13-Full-Source.zip'
