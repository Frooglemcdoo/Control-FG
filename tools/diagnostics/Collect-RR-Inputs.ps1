...
$output=Join-Path $ControlFGProjectRoot ('RR-Inputs-C1-'+(Get-Date -Format 'yyyyMMdd-HHmmss'))
[void](New-Item -ItemType Directory -Path $output)
foreach ($capture in $selected) { Copy-Item -LiteralPath $capture.FullName -Destination $output -Recurse }
& (Join-Path $ControlFGProjectRoot 'Collect-ControlFG-Compact-Logs.ps1') -OutputDirectory $output
$zip=$output+'.zip'
