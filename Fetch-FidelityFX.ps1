...
try {
    ...
    $apiHeader = Get-ChildItem -LiteralPath $expanded -Filter 'ffx_api.h' -File -Recurse | Select-Object -First 1
    if (-not $apiHeader) { throw 'Could not locate ffx_api.h.' }
    $fidelityRoot = $apiHeader.Directory.Parent.Parent.FullName
    $signedBin = Join-Path $fidelityRoot 'signedbin'
    if (-not (Test-Path -LiteralPath $signedBin)) { throw 'Could not locate FidelityFX signedbin.' }
