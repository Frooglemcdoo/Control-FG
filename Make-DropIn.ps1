...
try {
    ...
    if ($v.Version -ne $Version -or $v.AbiCheck -ne 'Passed') { throw 'Validated v1.0.0 build not found.' }
    ...
    $out=Join-Path $releaseRoot ('Control-FG-v'+$Version)
    $publicZip=Join-Path $releaseRoot ('Control-FG-v'+$Version+'.zip')
    $legacyZip=Join-Path $PSScriptRoot ('Control-FG-v'+$Version+'-DropIn.zip')
    ...
    foreach ($doc in @('INSTALL.md','TROUBLESHOOTING.md','RELEASE_NOTES.md','Collect-ControlFG-Logs.cmd','Collect-ControlFG-Logs.ps1')) {
    ...
    foreach ($doc in @('STREAMLINE-LICENSE.txt','THIRD-PARTY-NOTICES.md')) {
...
INSTALL
1. Copy dxgi.dll and ControlFGStreamline beside Control_DX12.exe.
2. Launch Control in DirectX 12 mode.
3. Press F10 to open or close the overlay.

RTX 40-series: Off + 2x only.
Supported release target: Control Steam DX12 build 21225456.
See INSTALL.md and TROUBLESHOOTING.md for details.
